...
import { routers } from "@/frontend/constants/routers";
import { useSession } from "next-auth/react";
import { wallets } from "@/frontend/constants/wallets";
import { redirect, useRouter } from "next/navigation";
import { useEffect, useState } from "react";
import { SocialIcon } from "react-social-icons";
import Network from "@/frontend/components/network";
import Wallet from "@/frontend/components/wallet";
import { WalletType } from "@/types";
import { networks } from "@/frontend/constants/networks";
import { APP_NETWORK } from "@/frontend/constants/enviroments";
...
export default function SignIn() {
    ...
    return (
        <div className="min-h-screen bg-slate-950 dark:from-slate-950 dark:via-slate-900 dark:to-slate-950">
            <div className="mx-auto my-0 flex h-full w-full  flex-col">
                <header className="z-10 p-4 border-b border-slate-700 bg-slate-900/80 dark:bg-slate-900/80 backdrop-blur-md">
                        ...
                        <div className="flex h-14 items-center justify-between">
                            <section className="flex items-center">
                                <Link href={routers.home} className="flex items-center gap-3">
                                    ...
                                    <h3 className="text-2xl font-bold text-white dark:text-white font-stretch-50%">CIP68</h3>
                                </Link>
                            </section>
                ...
                <main className="mt-[60px] mb-[20px] flex items-center justify-center flex-col h-full">
                    <section className="w-[540px] box-border py-[35px] px-[45px] bg-slate-900 dark:bg-slate-900 shadow-lg rounded-md border border-slate-700 dark:border-slate-700">
                        <div className="flex items-center justify-between">
                            <div className="text-[20px] text-white dark:text-gray-100">
                                <strong>Connect Wallet</strong>
                            </div>
                            <div className="w-[120px] text-[16px] text-end text-slate-300 dark:text-gray-300">
                                <strong>{network.charAt(0).toUpperCase() + network.slice(1).toLowerCase()}</strong>
                            </div>
                        </div>

                        <div className="flex mt-5">
                            <section className="pr-[30px] border-r flex flex-col gap-5 border-solid border-slate-700 dark:border-slate-700 mr-[30px] h-[230px] overflow-y-auto overflow-x-hidden">
                ...
                <footer className="pb-[20px] text-center">
                    ...
                    <p className="mt-[16px] text-slate-400">CIP68 Network Foundation Cardano2vn</p>
                </footer>
            </div>
        </div>
    );
