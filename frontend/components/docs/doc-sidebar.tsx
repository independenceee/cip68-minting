...
import { docCategories, DocCategory, DocSidebarProps } from "@/frontend/constants/docs";
import { useState } from "react";

export default function DocSidebar({ onClose }: DocSidebarProps) {
    const pathname = usePathname();
    const [expandedSections, setExpandedSections] = useState<string[]>(["new-to-cardano", "learn", "explore-more", "cardano-evolution"]);
    ...
    return (
        ...
        >
            ...
            <div className="lg:hidden flex justify-end mb-4">
                <button onClick={onClose} className="p-2 text-gray-500 hover:text-gray-700" aria-label="Close sidebar">
                    <X className="w-6 h-6" />
                </button>
            </div>
            ...
            <nav className="space-y-1">
                {docCategories.map((category: DocCategory) => (
                    <div key={category.id} className="mb-4 lg:mb-6">
                        {category.title && (
                            <h3 className="text-xs lg:text-sm font-semibold text-gray-500 uppercase tracking-wide mb-2 lg:mb-3">{category.title}</h3>
                        )}
                                ...
                                category.sections.map((section) => (
                                    <li key={section.id}>
                                        ...
                                        >
                                            ...
                                            {section.external && <ExternalLink className="h-3 w-3 lg:h-4 lg:w-4 ml-2" />}
                                        </Link>
                                    </li>
                                ))
                            )}
