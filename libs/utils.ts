...
export async function datumToJson(
  ...
  datumMap.forEach((value, key) => {
    ...
    if (keyStr === "author" && !option?.containauthor) {
      return;
    }
    obj[keyStr] = keyStr !== "author" ? value.toString("utf-8") : value.toString("hex");
  });
  return obj;
}
...
export function convertToKeyValue(data: { k: { bytes: string }; v: { bytes: string } }[]): Record<string, string> {
  return Object.fromEntries(
    data.map(({ k, v }) => {
      ...
      const value = key === "author" ? v.bytes : Buffer.from(v.bytes, "hex").toString("utf-8");
      return [key, value];
    }),
  );
}